...
output_dir = os.path.join(valid_dir,'analysis','9_plot_event_Q_combine')
...
events = ['06/30/2011', '06/08/2017', '06/17/2018']

# define line colors
colors = ['magenta','red','lightsalmon','orange','blue','royalblue','dodgerblue','green']
# colors = ['purple','red','coral','orange', 'blue','dodgerblue','c','green']
marks = ["v", "s", "D", "P", "s", "D", "P", "*"]
linestyles = ['-', '-', '--', ':', '-', '--', ':', '-']
...
for event in events:
    ...
    nrow, ncol = 1, 1
    lwd, ms = 1, 0 # line width, mark size.
    
    fig, ax = plt.subplots(nrow, ncol,figsize=[5.5, 7.08*0.35], constrained_layout=True)
    fig.suptitle(event_format_str+' Event Hydrograph', fontsize='small',weight='semibold')
    
    # plot obs
    df_obs_eval = df_obs.truncate(before=eventStartDate, after=eventEndDate)
    ax.plot(df_obs_eval.index, df_obs_eval['obs'], color='black', marker='o', linestyle='solid', 
            label='obs', linewidth=lwd*1.5, markersize=ms*1.5)
    
    # plot sim of different complexity levels
    for comp_id in range(len(levelArray)): 
        ...
        ax.plot(df_sim_eval.index, df_sim_eval['sim'], linestyle=linestyles[comp_id], marker=marks[comp_id], markersize=ms, color=colors[comp_id],
                label=complexity_level, linewidth=lwd)     #

        ax.set_xlabel('Date [mm-dd, %d]'%(event_yr),fontsize='small')
        ax.set_ylabel('Flow (cms)',fontsize='small')

        date_form = DateFormatter("%m-%d") #"%Y-%m-%d"
        ax.xaxis.set_major_formatter(date_form)
        plt.xticks(rotation = 15) #'vertical'
        ax.tick_params(axis='both', direction='out', labelsize='small')

        ax.legend(fontsize='small',ncol=2)
    ofile = event_format_str+'_hydrograph.png'
