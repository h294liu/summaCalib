...
# Note: This code plots the complete trace of samples because it reads sampels from ostModel.txt.
...
if __name__ == '__main__':
    ...
    # an example: python 1_plot_param_trace.py ../control_active.txt 1 2
...
#     control_file = '/home/h294liu/project/proj/5_summaCalib/5_calib_test2/BowAtBanff9/calib/control_active.txt'
#     experiment_id = [1,2]
    ...
    output_path = os.path.join(analysis_path, '2_plot_param_trace_complete')
    ...
    ofile_fig = os.path.join(output_path, 'experiment%s_trace_complete.png'%(experiment_id_str))  # output plot figure
    ofile_txt = os.path.join(output_path, 'experiment%s_summary_ostModel.txt'%(experiment_id_str))  # output best param information
    ...
    i = experiment_num-1 # Note: OstModel.txt has cumulative records, so only need to read the last experiment ostModel.txt.
    ...
    ostModel = os.path.join(archive_path, 'OstModel0.txt')

    # read ostModel
    sample_ids,objs,multp_values = read_param_sample_from_ostModel(ostModel)
        ...
        for j in range(col_num):
            ...
            if subplot_count <= Nparam: 
                ...
                if i==0 and j==0: 
                    ...
                    #  if there are multiple best param sets, and all best param values are the same, save a sing param value.
