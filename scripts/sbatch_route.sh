...
# (1) Merge daily outputs into one file. Parellel Python code.
python $calib_path/scripts/3b_concat_split_summa.py $control_file outputPath day

# # (2) Change negative summa route runoff to 0 (not -9999 or any negative value). 
# ncap2 -h -O -s 'where(averageRoutedRunoff_mean<0.) averageRoutedRunoff_mean=0.0;' $summa_outputPath/${summa_outFilePrefix}_day.nc $summa_outputPath/${summa_outFilePrefix}_day.nc 

# (3) Add fillvalue to summa route output
ncatted -h -a _FillValue,,o,d,-9999.0 $summa_outputPath/${summa_outFilePrefix}_day.nc 

# (4) Shift summa output time back 1 day for routing - only if computing daily outputs!
...
# # ----------------------------------------------------------------------------
# --- 4.  save the best results                                              ---
# ------------------------------------------------------------------------------
echo save best
date | awk '{printf("%s: save best\n",$0)}' >> $calib_path/timetrack.log
python save_best.py $control_file

# # ------------------------------------------------------------------------------
# # --- 5.  save the current param and generate a new param based on DDS       ---
# # ------------------------------------------------------------------------------
# echo generate parameter sample
# date | awk '{printf("%s: generate parameter sample\n",$0)}' >> $calib_path/timetrack.log

# python DDS.py multiplier_bounds.txt trial_stats.txt $1 $2 UseInitialParamValues 0 tpl/multipliers.tpl multipliers.txt OstModel0.txt

# # ------------------------------------------------------------------------------
# # --- 6.  delete summa outputs for the next iteration                        ---
# # ------------------------------------------------------------------------------
# echo delete summa outputs
# date | awk '{printf("%s: delete summa outputs\n",$0)}' >> $calib_path/timetrack.log
...
# 7.  Merge hourly outputs into one file.
$calib_path/scripts/3b_concat_split_summa_timestep.sh
