...
def process_command_line():
    ...
    parser.add_argument('param_bounds_file', help='input. file storing param range.')
    ...
    parser.add_argument('max_iterations', help='input. max iteration.')
    parser.add_argument('initial_option', help="initial value option: 'UseInitialParamValues' or 'UseRandomParamValues'.")
    parser.add_argument('warm_start', help='whether use the the best param set of the existing record file. 1: use. 0: not use.')
    ...
    parser.add_argument('param_record_file', help='input & output. param record file, in format of OstModel.txt.')
    args = parser.parse_args()
    return(args)

if __name__ == "__main__":
    
    # Example: python DDS_sample.py multiplier_bounds.txt trial_stats.txt 1 10 UseInitialParamValues 0 tpl/multipliers.tpl multipliers.txt OstModel0.txt

    # process command line 
    # check args
    if len(sys.argv) != 10:
        print("Usage: %s <param_bounds_file> <obj_file> <i_iteration> <max_iterations> <initial_option> <warm_start> <param_tpl_file> <param_file> <param_record_file>" % sys.argv[0])
        sys.exit(0)
    ...
    warm_start = int(args.warm_start)                         # input. whether use the the best param set of the existing record file. 1: use. 0: not use.
    ...
    param_record_file = args.param_record_file           # input & output. param record file, in format of OstModel.txt.

#     param_bounds_file = 'multiplier_bounds.txt'          # input. file storing param range
#     obj_file = 'trial_stats.txt'                         # input. file storing obj function 
#     iteration_idx = int(1)                               # input. current iteration id starting from 1
#     max_iterations = int(10)                             # input. max iteration.
#     initial_option = 'UseInitialParamValues'             # input. initial value option: 'UseInitialParamValues' or 'UseRandomParamValues'
#     warm_start = 0                                       # input. whether use the the best param set of the existing record file. 1: use. 0: not use.
#     param_tpl_file = 'tpl/multipliers.tpl'               # input. param template file where param value is replaced by param name.
#     param_file = 'multipliers.txt'                       # input & output. one set of param sample.
#     param_record_file = 'OstModel0.txt'                  # input & output. param record file, in format of OstModel.txt.
    ...
    if iteration_idx==1:
        ...
        if warm_start==1 and os.path.exists(param_record_file):
    ...
    # =======================================================================
    # Define the next iteration param set
    # =======================================================================
    elif iteration_idx>1:
        
        # -----------------------------------------------------------------------
        # Save the previous param and obj 
        # -----------------------------------------------------------------------
        # Read param sample and obj function from the previous iteration
        ...
        if not os.path.exists(obj_file):
            ...
            quit()
        obj_previous = np.loadtxt(obj_file, usecols=[0])[0] * (-1) # obj = negative KGE 
        print('Run=%d. Obj=%.6E'%(iteration_idx,obj_previous))
        
        # Save previous param and obj to param_record_file
        if not os.path.exists(param_record_file): # write a new record file
            with open(param_record_file, 'w') as f:
                ...
                f.write('1\t%.6e\t'%(obj_previous))
                for i_param in range(param_dim):
                    f.write('%.6e\t'%(param_sample[i_param]))
                f.write('\n')            
        
        else: # append to an existing record file
            param_record_df = pd.read_csv(param_record_file, header='infer', skip_blank_lines=True,
                                          delim_whitespace=True, engine='python')
            previous_run_count = param_record_df['Run'].iloc[-1]
            with open(param_record_file, 'a') as f:
                # write the i^th obj and param values
                f.write('%d\t%.6e\t'%(previous_run_count+1, obj_previous))
                for i_param in range(param_dim):
                    f.write('%.6e\t'%(param_sample[i_param]))
                f.write('\n')                    
        
        # -----------------------------------------------------------------------
        # Generate a new sample of param set based on DDS
        ...
        np.copyto(param_sample,param_sample_previous) #(dst,src)
    ...
    # =======================================================================
    # Output the new sample of param set
    # =======================================================================
