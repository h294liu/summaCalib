...
# WARNING: This script will not work if there are no (time,gru) or (time,hru) dimensioned varibales.
# For example, variables with dimension (gru), (depth,hru) will not be processeed correctly in the step of reordering dimensions.
...
gru1=gru
gru2=gruId
gru3=routingGammaShape
gru4=routingGammaScale

# loop through files
echo Splitting gru and hru variables
for oldFile in $summa_outputPath/${summa_outFilePrefix}*G*_timestep.nc ; do
     ...
     # Note: ensure time dimension existing in both gru and hru files.
     ncks -h -O    -C -v${gru1},${gru2},${gru3},${gru4},time $oldFile $gruFile
     ncks -h -O -x -C -v${gru1},${gru2},${gru3},${gru4} $oldFile $hruFile
...
combineFile=$summa_outFilePrefix\_timestep.nc
...
cp ${hruPath}/${combineFile} $summa_outputPath/$summa_outFilePrefix\_timestep.nc
ncks -h -A ${gruPath}/${combineFile} $summa_outputPath/$summa_outFilePrefix\_timestep.nc
