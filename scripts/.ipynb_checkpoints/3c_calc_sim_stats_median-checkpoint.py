...
if __name__ == '__main__':
    ...
    # --- loop gauge to calculate KGE ---
    gauge_file = '/home/h294liu/project/proj/7_nelson/scripts/1_find_gauge_upstream_GRUs/gauge_phase_0&1_withinGRU.csv'
    gauge_df = pd.read_csv(gauge_file, na_values=["-99.0","-999.0","-9999.0","NA"],usecols=['Gauge_ID','withinGRU'])    
    stat_records = [] # 3 columns: gaugeId, GruId, KGE.
    
    for index, row in gauge_df.iterrows():
        Gauge_ID = row['Gauge_ID']        
        gru = row['withinGRU']  
        
        # extract gru's simulated flow.
        sim  = f[simVarName][:,f['reachID']==gru].values[:,0] #(time, segments)
        df_sim = pd.DataFrame({'sim':sim},index = time)
        df_sim.index = pd.to_datetime(df_sim.index)

        # extract gru's observed flow.
        df_obs = pd.read_csv(obs_file, index_col='Date', na_values=["-99.0","-999.0","-9999.0","NA"],
                             usecols=['Date',Gauge_ID],parse_dates=True, infer_datetime_format=True)  
        df_obs.columns = ['obs']
...
#         rmse = get_RMSE(obs=df_merge['obs'].values, sim=df_merge['sim'].values)
        ...
        # --- record results ---
        stat_records.append(kge)
        
    gauge_df['KGE'] = stat_records
    
    # --- save --- 
    f = open(stat_output, 'w+')
    f.write('%.6f' %gauge_df['KGE'].median() + '\t# Median KGE\n')
    f.write('\n')
    
    # print each gauge statistical details as well.
    f.write('Gauge_ID,withinGRU,KGE\n')
    for index, row in gauge_df.iterrows():
        f.write('%s,%s,%.6f\n'%(row['Gauge_ID'], row['withinGRU'], row['KGE']))
    f.close()
