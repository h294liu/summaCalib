...
if __name__ == '__main__':
    ...
    # #### 3. Update complexity level
    fin = open(summa_filemanager, "rt") #input file    
    summa_filemanager_tmp = read_from_control(control_file, 'summa_filemanager') # temporary output file
    fout = open(summa_filemanager_tmp, "wt")
    
    for line in fin:        
        fout.write(line.replace('LEV', complexity_level)) #read replace the string and write to output file
        
    #close input and output files
    fin.close()
    fout.close()    
    shutil.move(summa_filemanager_tmp, summa_filemanager)
     
    # #### 4. Read a priori param values
    ...
    # #### 5. Determine multiplier lower/upper bounds
...
#             if param_name != 'routingGammaScale': 
            ...
            # Note: the special treatment of routingGammaScale is disabled here just for this complexity project.
...
#                 param_max = (GRU_channel_length/v_lower)/max(shape_priori)
    ...
    # #### 6. Save param and multiplier information into text.
